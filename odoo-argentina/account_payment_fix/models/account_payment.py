    ...
    def _onchange_journal(self):
        ...
        if self.journal_id:
            ...
            if not self.reconciled_bill_ids:
                ...
                moves= self.env['account.move'].search_count([('payment_id.payment_group_id','=', self.payment_group_id._origin.id)])
                #moves= self.env['account.move'].search([('state','=', 'draft')])
                _logger.info('payment_group_ids origin:' + str(self.payment_group_id._origin.id))
                _logger.info('payment_group_ids str:' + str(self.payment_group_id._origin.id))
